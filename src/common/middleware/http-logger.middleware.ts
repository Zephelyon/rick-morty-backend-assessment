...
function extractGraphQLOperationName(query: any): string | undefined {
  ...
  const q = query
    ...
    .trim();
  const match = q.match(
    /^(query|mutation|subscription)\s+([A-Za-z_][A-Za-z0-9_]*)\b/,
  );
  ...
  use(req: Request, res: Response, next: NextFunction) {
    ...
    const finish = () => {
        ...
        if (body) {
          if (url.startsWith('/api/rick-and-morty')) {
            if (isGraphQLIntrospection(body)) {
              ...
              if (
                typeof gqlPayload.query === 'string' &&
                gqlPayload.query.length > 700
              ) {
      ...
      if (resolvedOperationName) {
        metaParts.push(
          `${chalk.gray('op')} ${chalk.white(resolvedOperationName)}`,
        );
      }
